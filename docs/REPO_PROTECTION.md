...
`pkg/pocket/Platforms/_images/paprium.bin` is the core's platform artwork. The
shipping banner is by [Lemongrwab](https://github.com/Lemongrwab), used with
permission (converted to Analogue's 521x165 blue house-style `.bin`). The
full-resolution source stays out of the repository on the same principle that
keeps the ROM out; the downscaled platform image is **kept** - decided
2026-08-31, credit updated 2026-09-07 - because the core needs it to display
and it is the artwork in its functional form rather than a redistributable
