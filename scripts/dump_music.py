...
Header, as far as it is understood:
    ...
    +0x10  26 bytes  per-voice array A - volume, 0x10 in every module
    +0x2A  26 bytes  per-voice array B - varies per track
    +0x44  26 bytes  per-voice array C - zero in every module
...
def main():
    ...
    for t in range(1, count + 1):
        ...
        live += 1
        title = bytes(c ^ 0xA5 for c in u[0x78:0x98]).rstrip(b'\x00')
        print("  track %-3d 0x%06X -> %5d bytes  %s"
              % (t, a, len(u), title.decode('ascii', 'replace')))
