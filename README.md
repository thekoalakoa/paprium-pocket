...
| Fix | Was |
|---|---|
| A dropped weapon kept spinning instead of settling on the ground | Knives `0xDC`, chains `0xDD`, neon sticks `0xDE` and pipes `0xE0` spun where they should have settled. The game hands a dropped item over and walks away: it arms the ground animation as a queued follow-up and never writes the animation itself again, for hundreds of frames. The rule that refused the switch is the one that stops a character sliding in its standing pose at a screen transition (`PPM_CHAIN_ONLY_AT_END`, 0.2.1), and measured across 12 minutes of play it fired on **0 of 821** queue requests — in practice "never chain". Three readings of *when* the game arms the queue were tried against that, and each one that settled the weapons also put a walking character back into its standing pose, because the game uses the same idiom for a player's looping idle as for a dropped knife. What separates them is not the timing but the object: the firmware's own per-object animation count splits props (2 to 29 animations, every weapon is 10) from actors (39 to 220, the characters are 157) with no overlap. A prop now takes its queued follow-up at any animation end — the rule hardware confirmed settles every weapon — while an actor keeps the 0.2.4 rule unchanged. The frame-word flag bits, `obj_data+0x06`, object movement, a frame clock, and copying the reference emulator were each measured and refuted first; see `patches/README.md` |
| The Boom Box’s bar graph lit up permanently instead of following the music | **Boom Box level meter** (0.2.3). Cartridge RAM `0x1B98– 0x1BFF` is a per-voice level feed — 26 voices of left/right, ending exactly on `0x1C00` — that the cartridge’s music engine rewrites every row, and the Boom Box draws its 26 bars straight from it. This port substitutes the album for the synthesiser and never sequences the modules, so nothing wrote that window at all and the game drew its bars out of uninitialised memory. The firmware already decompresses the sequence module on every music command and then discards it, so the bars are now driven from that — the real per-voice arrangement, at the module’s own tempo, for no extra work. The tempo comes from the module header, verified against hardware: Stage Clear is 105 rows at 20 rows a second, 5.25 seconds, matching a capture of the Boom Box exactly |
...
this core by **LemonGrwab**, who supplies it in Analogue's format. The artwork
is theirs. `scripts/make_platform_image.py` converts a PNG into that format for
anyone who wants to start from one.
