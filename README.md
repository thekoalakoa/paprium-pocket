...
| Fix | Was |
|---|---|
| A dropped weapon kept spinning instead of settling on the ground | Enemies drop a knife `0xDC`, electric stick `0xDE` or pipe `0xE0`, and each spun where it should have settled. The game hands the item over and walks away: it arms the ground animation as a queued follow-up and never writes the animation itself, for hundreds of frames. The rule that refused the switch was the one that stops a character sliding in its standing pose at a screen transition (`PPM_CHAIN_ONLY_AT_END`, 0.2.1), and measured across 12 minutes of captured play it fired on **0 of 821** queue requests — in practice "never chain". What separates the two cases is *when* the game arms the queue: **on the frame the animation ends** when it means "switch now" (the weapons, 11 of 11) and mid-cycle when it is only a fallback it will resolve itself (the walk-in, 0 of 31). `PPM_CHAIN_FRESH_QUEUE` takes a queued follow-up at a looping end only when the queue was armed at that end. The frame-word flag bits, `obj_data+0x06`, object movement, and copying the reference emulator were each measured and refuted first; see `patches/README.md` |
