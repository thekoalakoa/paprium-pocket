...
| Fix | Was |
|---|---|
| A dropped weapon kept spinning instead of settling on the ground | Knives `0xDC`, chains `0xDD`, neon sticks `0xDE` and pipes `0xE0` spun where they should have settled. The game hands a dropped item over and walks away: it arms the ground animation as a queued follow-up and never writes the animation itself, for hundreds of frames. The rule that refused the switch was the one that stops a character sliding in its standing pose at a screen transition (`PPM_CHAIN_ONLY_AT_END`, 0.2.1), and measured across 12 minutes of captured play it fired on **0 of 821** queue requests — in practice "never chain". The game arms a queue in three places, and 0.2.4 caught only one of them: **at an animation's end** ("switch now"), **on the same call that set the animation** ("play this, then that" — an end a whole animation away), and mid-cycle, where a character's queue is a transient the game resolves itself within 41 draws while a dropped item's stands for 82 to 458 and is never touched again. 0.2.5 chains on all three: the end, the load, and a queue left standing past 64 draws. The frame-word flag bits, `obj_data+0x06`, object movement, a frame clock, and copying the reference emulator were each measured and refuted first; see `patches/README.md` |
