...
Three. Everything else that was listed here has moved to
...
| A dropped weapon keeps spinning instead of settling on the ground | **Open (0.2.3).** Enemies drop a knife, pipe or electric stick; each has a short "falling" animation that cycles and a long ground animation that is mostly still and stirs every few seconds. The item is asking for the ground animation — a hardware capture caught object `0xDE` playing its fall with the ground animation queued — and this port refuses the switch. The rule that refuses it is the same one that stops a character sliding in its standing pose at a screen transition (`PPM_CHAIN_ONLY_AT_END`, 0.2.1): switching it off fixes the weapons and brings the sliding back, confirmed on hardware both ways. The two cases are identical in every field the firmware reads, so telling them apart needs something it currently discards — the per-frame flag bits the reference emulator ignores too, which is why that emulator shows the sliding as well. Cosmetic, and only on dropped items. Tracked in `docs/PORT_PLAN.md` |
