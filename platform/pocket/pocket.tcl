...
# ==============================================================================
# From agg23/openfpga-NES platform/pocket/pocket.tcl (CC0-1.0, OpenGateware
# authors). The rest of platform/pocket/ is Analogue stock and so carries no
# markers; this file is not stock (core-template has no pocket.tcl, it inlines
# these assignments in src/fpga/ap_core.qsf), so the local deltas below are
# marked like anywhere else.
#
...
# pocket: agg23 spells this TIMEQUEST_MULTICORNER_ANALYSIS, the pre-20.1 name.
# core-template already uses the current one, so this follows core-template.
...
# ==============================================================================
# pocket: agg23 leaves this ON because it vendors an stp1.stp alongside. No
# .stp is vendored here, and SignalTap ON without one only costs fitter time.
...
# SDRAM DQ: force output enable into IOE fast register to reduce OE routing delay
...
# SDRAM DQ: force data output into IOE fast register to reduce data routing delay
...
# SDRAM address/command: force output into IOE fast register
...
# pocket: cke is the one command pin agg23 left out of its own list. It is
# launched by the same register stage as the rest, so it belongs with them.
...
# SDRAM DQ: force input data into IOE fast register to minimize read path delay
...
# Scripts
# ==============================================================================
# pocket: agg23 also sets a POST_FLOW_SCRIPT_FILE pointing at build_cdf.tcl,
# which writes a JTAG chain description for its own programming step. That file
# is not vendored and nothing here programs over JTAG, so the hook is dropped.
